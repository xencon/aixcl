...
Query to list OpenCode plugin conversations from the opencode database.
...
**Note:** OpenCode plugin conversations are stored in the `opencode` database (separate from `webui`). To query OpenCode conversations, use `check_db.sh` or query the `opencode` database directly.
