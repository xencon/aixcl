#!/bin/sh
# Vault Agent for Grafana bootstrap password (KV store)
...
fetch_bootstrap_password() {
    log "Fetching Grafana bootstrap password from Vault KV..."
    
    # Use wget to read from KV store (curl not available in Vault image)
    response=$(wget -qO- "${VAULT_ADDR}/v1/kv/data/bootstrap/grafana" \
    ...
    if ! echo "$password" > /run/secrets/grafana-password; then
        log "ERROR: Cannot write /run/secrets/grafana-password"
        return 1
    fi
    if ! chmod 644 /run/secrets/grafana-password; then
        log "ERROR: Cannot chmod /run/secrets/grafana-password"
        return 1
    fi
    
    log "Bootstrap password written to /run/secrets/grafana-password"
}
