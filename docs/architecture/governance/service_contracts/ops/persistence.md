...
- **Current Implementation**: Only active profiles (`ops`, `sys`) include PostgreSQL for persistence. The `ops` profile includes PostgreSQL as part of the observability stack, while `sys` includes it along with the full UI and monitoring suite.
